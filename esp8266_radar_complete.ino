...
const char* deviceName = "ESP8266-Radar-Complete";
...
void createAlert(String type, String message, int objectCount) {
  ...
  lastAlert.expectedCount = initialObjectCount;
  alertActive = true;
  
  Serial.printf("ALERTE API: %s (Objets: %d/%d)\n", 
                message.c_str(), objectCount, initialObjectCount);
}
...
void loop() {
  ...
  for (int x = 160; x >= 20; x--) {
    ...
    // Mettre à jour le compteur pour l'API
    currentData.objectCount = objectCount;

    Serial.print(x); Serial.print(","); Serial.print(distance); Serial.print(","); Serial.print(objectCount); Serial.println(".");
...
void setupWebServer() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "<title>ESP8266 Radar Complete</title>";
    html += "<style>";
    ...
    html += "<h1>ESP8266 Radar Complete</h1>";
